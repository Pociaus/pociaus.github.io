var quizController = (function(){
	...
	var adminFullName = ['Gvidas', 'Pocius'];
	...
	return{
		...
		createQuestionList: function(getQuestions) {
			...
			for(var i = 0; i < getQuestions.getQuestionCollection().length; i++){
				...
				domItems.insertedQuestWrapper.insertAdjacentHTML('afterbegin', questHTML);
		...
		clearQuestList: function(storageQuestList){
			...
			if(storageQuestList.getQuestionCollection().length > 0){
				var conf = confirm('Warning: You wil lose entire question list');
				if(conf){
		...
		newDesign: function(ansResult, selectedAnswer){
			...
			twoOptions = {
				...
				optionsSpanBg: ['rgba(200, 0, 0, 0.7)','rgba(0, 250, 0, 0.7)']
			};
		...
		for(var i=0; i< updatedOptionsDiv.length; i++){
			if(e.target.className === 'choice-'+ i){
				...
				selectedDomItems.nextQuestbtn.onclick = function() {
					nextQuestion(quizController.getQuestionLocalStorage, quizCtrl.getQuizProgress);
				}
			}
		}
